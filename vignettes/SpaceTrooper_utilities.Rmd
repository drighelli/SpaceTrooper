...
out <- safe_run(computeQScore(spe))
...
formula is generated as follows:
 ...
 must be written exactly as they appear here (e.g. `I(abs(log2AspectRatio) * as.numeric(dist_border < 50))`
 must have spaces around the `*` and `<` operators). However, the terms do not need 
 to follow any specific order. If the outliers for one or more metrics are not 
 sufficient, they will not be considered for model training and the corresponding terms 
 are automatically dropped from the formula.
