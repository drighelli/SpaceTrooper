...
project <- "S2-S3"
sample.name <- ""
field.of.view <-"LungR1"

### settings ###
  # main.path <- file.path("/Users","silviobicciato","Documents","GeneChip_Data",
                         # "AIRC_Piccolo","Nanostring_CosMx",project)
main.path <- file.path("~/Downloads/CosMx_Data/Lung5_Rep1/Lung5_Rep1-Flat_files_and_images/")
  ...
  if (!file.exists(main.result.dir)){dir.create(main.result.dir)}
  result.dir <- file.path(main.result.dir,"Lung5_Rep1")
...
####################################
  CosMx.obj <- LoadNanostring(data.dir = main.path, fov = field.of.view)
  ...
  fov.xdim <- 5472
  fov.ydim <- 3648
  pdf(file.path(paste0("01_",sample.name,"_FOVs.pdf")), useDingbats=FALSE)
  ...
  pdf(file.path(paste0("02_", sample.name, "_CellMarkersIF.pdf")),
