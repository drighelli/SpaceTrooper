---
title: "03_13_cosmx_human_tnbc_25027_prepro_metric_report"
...
table(valid_poly)
table(st_is_valid(polygons_whole, reason = T))
```
...
```{r}
variables <- c("total", "detected", "logtot", "um_area", "log2_AspectRatio", "tot_det_ratio", "control_counts", "custom_um_area", "Mean.DAPI")
...
```{r}
# the function is not suitable because it computes the outlier a priori from the distirbution min and max values
attr(spe$outlier_dapi, "threshold")
```
...
# First cell skimming with modified z score method {.tabset .tabset-fade .tabset-pills}

Using histograms to know the variable distribution and z-score method to identify outliers.
The z-score formula is modified to 
...
```{r}
negprob_z_score <- (spe$control_counts - median(spe$control_counts))/mad(spe$control_counts)
summary(negprob_z_score)
```

```{r, out.width="150%", out.height="150%"}
spe$negprob_outliers <- ifelse(negprob_z_score > 3 | negprob_z_score < -3, TRUE, FALSE)

polygons_whole$negprob_outliers <- ifelse(spe$negprob_outliers == TRUE, "red", "grey80")

tm_shape(polygons_whole) + 
  tm_fill(col= "negprob_outliers") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
            main.title = paste0("Outlier cells for negative probe counts", 
                                "\nFlagged cells = ",
                                table(spe$negprob_outliers)["TRUE"], " out of ", dim(spe)[2], ", ",
                                round(table(spe$negprob_outliers)["TRUE"]/dim(spe)[2]*100, 2), "% of total cells"),
            main.title.fontface = 2,
            main.title.size = 1, 
            inner.margins = c(0, 0, 0, 0),
            outer.margins = c(0, 0, 0, 0))
...
```{r}
hist(negprob_z_score)
```

```{r}
negprob_spe <- spe[,spe$negprob_outliers == FALSE]
summary(negprob_spe$control_counts)
```
...
```{r}
area_z_score <- (spe$custom_area - median(spe$custom_area))/mad(spe$custom_area)
summary(area_z_score)
```

```{r, out.width="150%", out.height="150%"}
spe$area_outliers <- ifelse(area_z_score > 3 | area_z_score < -3, TRUE, FALSE)

polygons_whole$area_outliers<- ifelse(spe$area_outliers == TRUE, "red", "grey80")

tm_shape(polygons_whole) + 
  tm_fill(col= "area_outliers") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
            main.title = paste0("Outlier cells for custom um area", 
                                "\nFlagged cells = ",
                                table(spe$area_outliers)["TRUE"], " out of ", dim(spe)[2], ", ",
                                round(table(spe$area_outliers)["TRUE"]/dim(spe)[2]*100, 2), "% of total cells"),
            main.title.fontface = 2,
            main.title.size = 1, 
            inner.margins = c(0, 0, 0, 0),
            outer.margins = c(0, 0, 0, 0))
...
```{r}
area_spe <- spe[, spe$area_outliers == FALSE]
summary(area_spe$custom_um_area)
```
...
```{r}
dapi_z_score <- (spe$Mean.DAPI - median(spe$Mean.DAPI))/mad(spe$Mean.DAPI)
summary(dapi_z_score)
```

```{r, out.width="150%", out.height="150%"}
spe$dapi_outliers <- ifelse(dapi_z_score > 3 | dapi_z_score < -3, TRUE, FALSE)

polygons_whole$dapi_outliers <- ifelse(spe$dapi_outliers == TRUE, "red", "grey80")

tm_shape(polygons_whole) + 
  tm_fill(col= "dapi_outliers") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
            main.title = paste0("Outlier cells for DAPI signal", 
                                "\nFlagged cells = ",
                                table(spe$dapi_outliers)["TRUE"], " out of ", dim(spe)[2], ", ",
                                round(table(spe$dapi_outliers)["TRUE"]/dim(spe)[2]*100, 2), "% of total cells"),
            main.title.fontface = 2,
            main.title.size = 1, 
            inner.margins = c(0, 0, 0, 0),
            outer.margins = c(0, 0, 0, 0))
...
```{r}
dapi_spe <- spe[, spe$dapi_outliers == FALSE]
summary(dapi_spe$Mean.DAPI)
```
...
## Flagged cells according to control counts, custom um area and DAPI outliers

```{r}
sum(table(spe$negprob_outliers)["TRUE"], table(spe$area_outliers)["TRUE"], table(spe$dapi_outliers)["TRUE"])
```


# First cell skimming with (Hampel filter) on log-transformed data, (scuttle:isOutlier) {.tabset .tabset-fade .tabset-pills}

Using histograms to know the variable distribution and the median, plus or minus 3 median absolute deviations (MAD)

Control counts

```{r}
log2_control <- log2(spe$control_counts)
negprob_hampel <- scuttle::isOutlier(log2_control)
table(negprob_hampel)
```

Log-transformation does not allow to apply the Hampel filter

```{r}
summary(log2_control)
```
...
# Final report {.tabset .tabset-fade .tabset-pills}
...
# Flag score development {.tabset .tabset-fade .tabset-pills}
...
# Fov 72 {.tabset .tabset-fade .tabset-pills}
...
# Fov 66 {.tabset .tabset-fade .tabset-pills}
...
# Fov 146 {.tabset .tabset-fade .tabset-pills}
...
# Fov 20 {.tabset .tabset-fade .tabset-pills}
