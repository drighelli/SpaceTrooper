---
title: "05_08_mouse_brain_hemi_auto_first_flag_mc"
...
spe$polygons_whole$dapi_outlier <- outlier_color
...
tm_shape(spe$polygons_whole) + 
  ...
  tm_layout(legend.outside = TRUE,
            ...
            main.title = paste0("Outlier cells for dapi", 
                                "\n fence = " , round(out$fence, 2),
                                "\nFlagged cells = ",
                                table(spe$polygons_whole$dapi_outlier)["red"], " out of ", dim(spe)[2], ", ",
                                round(table(spe$polygons_whole$dapi_outlier)["red"]/dim(spe)[2]*100, 2), "% of total cells"),
            main.title.fontface = 2,
            main.title.size = 1, 
            inner.margins = c(0, 0, 0, 0),
            outer.margins = c(0, 0, 0, 0))
...
```{r}
dapi_poly <- spe$polygons_whole[spe$polygons_whole$dapi_outlier == "grey80",]
...
```{r}
dapi_spe <- spe[, spe$polygons_whole$dapi_outlier == "grey80"]
dim(dapi_spe)
```
...
flagged_cells <- c(sum(spe$polygons_whole$flagged_0total=="red", na.rm = T),
                   ...
                   sum(spe$polygons_whole$dapi_outlier=="red",na.rm = T))
...
unflagged_cells <- c(sum(spe$polygons_whole$flagged_0total=="grey80", na.rm = T),
                     ...
                     sum(spe$polygons_whole$dapi_outlier=="grey80",na.rm = T))
...
union_flagged <- length(unique(c(spe[,spe$polygons_whole$flagged_0total == "red"]$fov_id,
                   ...
                   spe[,spe$polygons_whole$umarea_outlier == "red"]$fov_id,
                   spe[,spe$polygons_whole$dapi_outlier == "red"]$fov_id)))
...
list_input <- list(flagged_0total = spe[,spe$polygons_whole$flagged_0total == "red"]$fov_id,
           ...
           umarea_outlier = spe[,spe$polygons_whole$umarea_outlier == "red"]$fov_id,
           dapi_outlier = spe[,spe$polygons_whole$dapi_outlier == "red"]$fov_id)
