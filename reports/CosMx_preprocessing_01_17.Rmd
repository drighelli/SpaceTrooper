...
df <- data.frame(Total_cell_number = dim(spe)[2], Total_transcript_counts = sum(spe$total), Total_detected_features = sum(spe$detected), Total_control_probe_counts = sum(spe$subsets_NegProb_sum), Median_transcript_counts_per_cell = median(spe$total), Median_detected_features = median(spe$detected),  Median_control_probe_counts = median(spe$subsets_NegProb_sum), Median_um_area = median(spe$um_area), Median_logged_height_width_ratio = median(spe$log2_AspectRatio), Median_total_counts_detected_features_ratio = median(spe$tot_det_ratio, na.rm = T),  Median_control_total_counts_ratio = median(spe$ctrl_total_ratio, na.rm = T))

kable(df, format = "html", col.names = c("Total cells", "Total counts", "Total features", "Total ctrl probes", "Median counts", "Median features", "Median ctrl",
                                         "Median area", "Median h/w", "Median counts/feat", "Median counts/ctrl"), align = "c", caption = "<center>Global summary metrics</center>",
      padding = 1L)
```
...
flag_data <- data.frame(Flagged_cells = as.integer(), Unflagged_cells = as.integer())
for(i in 1:length(flag_variables)){
  var <- grep(paste0("^", flag_variables[i],"$"), names(colData(spe)))
  list_i <- c(sum(colData(spe)[,var]==TRUE, na.rm = T), sum(colData(spe)[,var]==FALSE, na.rm = T))
  flag_data[i, ] <- list_i
}

rownames(flag_data) <- c("Total counts < 20%", " Total counts < 10%", "Total counts < 5%", "Features < 10%", "Features < 5%", "Area < 10%", "Area < 5%", 
...
kable(flag_data, format = "html", padding = 1L, align = "c", col.names = c("Flagged cells", "Unflagged cells"), caption = "<center>Flagged vs unflagged cells</center>")
```
