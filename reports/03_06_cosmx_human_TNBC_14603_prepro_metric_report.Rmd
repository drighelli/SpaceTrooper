---
title: "03_06_cosmx_human_tnbc_14603_prepro_metric_report"
...
ggplot() + 
  ...
  geom_text(aes(x = fov_positions$X_px+11/10*fov_size/2,
                ...
                label = fov_positions$fov), color="black", size = 2) +
  ggtitle(sample_name) +
...
```{r}
anno_file <- fread(file.path(dirname, "UAP7_14603_clusterAnnot@1_simple.txt"), sep = "\t", header = T)
...
"MG_cells" =	"dodgerblue4",
"CTLs" =	"deepskyblue",
"Bcells" =	"cyan",
"Tregs" =	"darkseagreen1",
"Tcells.Others" = "darkolivegreen1",
"Monocytes" =	"limegreen",
"Adipocytes" =	"green4",
"MHC2+cells" =	"darkgreen",
"Mixed_TAMs" =	"yellow4",
"TAMs" =	"yellow",
"AttractTAMs" = "greenyellow",
"OPN+TAMs" =	"olivedrab1",
"DCs" =	"seagreen2",
"Mast_cells" =	"lightyellow",
"Mix_BC_Plasma_cells" =	"goldenrod4",
...
"Lymph_ECs" =	"orchid4",
"Blood_ECs" =	"orchid1",
"LC3+cells" =	"lightcoral",
"MGP+BC_cells" =	"orangered1",
"ECM_BC_cells" =	"deeppink1",
"unknown" = "gray20",
"Mixed_BC_cells" =	"red4"
)
...
tm_shape(polygons_whole) + 
  ...
  tm_layout(legend.outside = TRUE,
            ...
            main.title = paste0("Negative probe counts above 95% threshold, \nvalue = ", 
...
tm_shape(polygons_whole) + 
  ...
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
            main.title = paste0("Negative probe counts above 99% threshold, \nvalue = ", 
                                round(quantile(colData(spe)$control_counts, probs = threshold, na.rm = T), 2), "\nFlagged cells = ",
...
tm_shape(polygons_whole) + 
  ...
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
            main.title = paste0("Custom area in um above 95% threshold, \nvalue = ", 
...
tm_shape(polygons_whole) + 
  ...
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
            main.title = paste0("Custom area in um above 99% threshold, \nvalue = ", 
                                round(quantile(colData(spe)$custom_um_area, probs = threshold, na.rm = T), 2), "\nFlagged cells = ",
...
count_df <- data.frame(count_data, color)
...
Local feature map of flag score, fov 132

```{r, out.width="150%", out.height="150%"}
polygons132 <- filter(polygons, fov==132) 

tm_shape(polygons132) + 
...
```{r, out.width="300%", out.height="900%"}
metric_color <- ifelse(polygons132$flag_score < quantile(spe$flag_score, probs = 0.15), "red", "white")
target_color <- ifelse(polygons132$target_counts < quantile(spe$target_counts, probs = 0.15), "red", "white")
count_area_ratio_color <- ifelse((polygons132$target_counts/polygons132$custom_um_area) < quantile((spe$target_counts/spe$custom_um_area), probs = 0.15), "red", "white")
area_color_lo <- ifelse(polygons132$custom_um_area < quantile(spe$custom_um_area, probs = 0.15), "red", "white")
area_color_hi <- ifelse(polygons132$custom_um_area > quantile(spe$custom_um_area, probs = 0.85), "red", "white")
log_hw_color_lo <- ifelse(polygons132$log2_AspectRatio < quantile(spe$log2_AspectRatio, probs = 0.15), "red", "white")
log_hw_color_hi <- ifelse(polygons132$log2_AspectRatio > quantile(spe$log2_AspectRatio, probs = 0.85), "red", "white")

polygons132 <- cbind(polygons132, metric_color = metric_color, target_color = target_color, count_area_ratio_color = count_area_ratio_color, area_color_lo = area_color_lo, area_color_hi = area_color_hi, log_hw_color_lo = log_hw_color_lo, log_hw_color_hi = log_hw_color_hi)

p1 <- tm_shape(polygons132) + 
  tm_fill(col= "metric_color", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
...
flagged_cells <- c(sum(polygons132$metric_color=="red", na.rm = T),
                   sum(polygons132$target_color=="red", na.rm = T),
                   sum(polygons132$count_area_ratio_color=="red", na.rm = T),
                   sum(polygons132$area_color_lo=="red", na.rm = T),
                   sum(polygons132$area_color_hi=="red", na.rm = T),
                   sum(polygons132$log_hw_color_lo=="red", na.rm = T),
                   sum(polygons132$log_hw_color_hi=="red", na.rm = T))

unflagged_cells <- c(sum(polygons132$metric_color=="white", na.rm = T),
                     sum(polygons132$target_color=="white", na.rm = T),
                     sum(polygons132$count_area_ratio_color=="white", na.rm = T),
                     sum(polygons132$area_color_lo=="white", na.rm = T),
                     sum(polygons132$area_color_hi=="white", na.rm = T),
                     sum(polygons132$log_hw_color_lo=="white", na.rm = T),
                     sum(polygons132$log_hw_color_hi=="white", na.rm = T))

percent_flagged <- round(flagged_cells/dim(polygons132)[1]*100, 2)

poly132 <- data.frame(threshold = c(0.15, 0.15, 0.15, 0.15, 0.85, 0.15, 0.85), flag_value = flag_value, unflagged_cells = unflagged_cells, flagged_cells = flagged_cells, percent_flagged = percent_flagged, row.names = c("Flag score", "Target counts", "Target counts/um area", "Custom area in um low", "Custom area in um high", "Logged aspect ratio low", "Logged aspect ratio high"))

kable(poly132, format = "html", col.names = c("Threshold", "Flag value", "Unflagged cells", "Flagged cells", "% flagged cells"), align = "c", caption = paste0("<center>Flagged cells by different metrics, total cells in fov: ", dim(polygons132)[1], ", total cells in whole sample: ", dim(polygons_whole)[1], ", flagged cells in whole sample: ", sum(spe$flag_score < quantile(spe$flag_score, probs = 0.15, na.rm = T)), "</center>"), padding = 1L)
```
...
Local feature map of flag score, fov 215

```{r, out.width="150%", out.height="150%"}
polygons215 <- filter(polygons, fov==215)

tm_shape(polygons215) + 
  tm_fill(col= "flag_score", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE)
...
```{r, out.width="300%", out.height="900%"}
metric_color <- ifelse(polygons215$flag_score < quantile(spe$flag_score, probs = 0.15), "red", "white")
target_color <- ifelse(polygons215$target_counts < quantile(spe$target_counts, probs = 0.15), "red", "white")
count_area_ratio_color <- ifelse((polygons215$target_counts/polygons215$custom_um_area) < quantile((spe$target_counts/spe$custom_um_area), probs = 0.15), "red", "white")
area_color_lo <- ifelse(polygons215$custom_um_area < quantile(spe$custom_um_area, probs = 0.15), "red", "white")
area_color_hi <- ifelse(polygons215$custom_um_area > quantile(spe$custom_um_area, probs = 0.85), "red", "white")
log_hw_color_lo <- ifelse(polygons215$log2_AspectRatio < quantile(spe$log2_AspectRatio, probs = 0.15), "red", "white")
log_hw_color_hi <- ifelse(polygons215$log2_AspectRatio > quantile(spe$log2_AspectRatio, probs = 0.90), "red", "white")

polygons215 <- cbind(polygons215, metric_color = metric_color, target_color = target_color, count_area_ratio_color = count_area_ratio_color, area_color_lo = area_color_lo, area_color_hi = area_color_hi, log_hw_color_lo = log_hw_color_lo, log_hw_color_hi = log_hw_color_hi)

p1 <- tm_shape(polygons215) + 
  tm_fill(col= "metric_color", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
...
flagged_cells <- c(sum(polygons215$metric_color=="red", na.rm = T),
                   sum(polygons215$target_color=="red", na.rm = T),
                   sum(polygons215$count_area_ratio_color=="red", na.rm = T),
                   sum(polygons215$area_color_lo=="red", na.rm = T),
                   sum(polygons215$area_color_hi=="red", na.rm = T),
                   sum(polygons215$log_hw_color_lo=="red", na.rm = T),
                   sum(polygons215$log_hw_color_hi=="red", na.rm = T))

unflagged_cells <- c(sum(polygons215$metric_color=="white", na.rm = T),
                     sum(polygons215$target_color=="white", na.rm = T),
                     sum(polygons215$count_area_ratio_color=="white", na.rm = T),
                     sum(polygons215$area_color_lo=="white", na.rm = T),
                     sum(polygons215$area_color_hi=="white", na.rm = T),
                     sum(polygons215$log_hw_color_lo=="white", na.rm = T),
                     sum(polygons215$log_hw_color_hi=="white", na.rm = T))

percent_flagged <- round(flagged_cells/dim(polygons215)[1]*100, 2)

poly215 <- data.frame(threshold = c(0.15, 0.15, 0.15, 0.15, 0.85, 0.15, 0.85), flag_value = flag_value, unflagged_cells = unflagged_cells, flagged_cells = flagged_cells, percent_flagged = percent_flagged, row.names = c("Flag score", "Target counts", "Target counts/um area", "Custom area in um low", "Custom area in um high", "Logged aspect ratio low", "Logged aspect ratio high"))


kable(poly215, format = "html", col.names = c("Threshold", "Flag value", "Unflagged cells", "Flagged cells", "% flagged cells"), align = "c", caption = paste0("<center>Flagged cells by different metrics, total cells in fov: ", dim(polygons215)[1], ", total cells in whole sample: ", dim(polygons)[1], ", flagged cells in whole sample: ", sum(spe$flag_score < quantile(spe$flag_score, probs = 0.15, na.rm = T)), "</center>"), padding = 1L)
```
...
Local feature map of flag score, fov 61

```{r, out.width="150%", out.height="150%"}
polygons61 <- filter(polygons, fov==61)
...
```{r, out.width="300%", out.height="900%"}
metric_color <- ifelse(polygons61$flag_score < quantile(spe$flag_score, probs = 0.15), "red", "white")
target_color <- ifelse(polygons61$target_counts < quantile(spe$target_counts, probs = 0.15), "red", "white")
count_area_ratio_color <- ifelse((polygons61$target_counts/polygons61$custom_um_area) < quantile((spe$target_counts/spe$custom_um_area), probs = 0.15), "red", "white")
area_color_lo <- ifelse(polygons61$custom_um_area < quantile(spe$custom_um_area, probs = 0.15), "red", "white")
area_color_hi <- ifelse(polygons61$custom_um_area > quantile(spe$custom_um_area, probs = 0.85), "red", "white")
log_hw_color_lo <- ifelse(polygons61$log2_AspectRatio < quantile(spe$log2_AspectRatio, probs = 0.15), "red", "white")
log_hw_color_hi <- ifelse(polygons61$log2_AspectRatio > quantile(spe$log2_AspectRatio, probs = 0.90), "red", "white")

polygons61 <- cbind(polygons61, metric_color = metric_color, target_color = target_color, count_area_ratio_color = count_area_ratio_color, area_color_lo = area_color_lo, area_color_hi = area_color_hi, log_hw_color_lo = log_hw_color_lo, log_hw_color_hi = log_hw_color_hi)

p1 <- tm_shape(polygons61) + 
  tm_fill(col= "metric_color", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
...
flagged_cells <- c(sum(polygons61$metric_color=="red", na.rm = T),
                   sum(polygons61$target_color=="red", na.rm = T),
                   sum(polygons61$count_area_ratio_color=="red", na.rm = T),
                   sum(polygons61$area_color_lo=="red", na.rm = T),
                   sum(polygons61$area_color_hi=="red", na.rm = T),
                   sum(polygons61$log_hw_color_lo=="red", na.rm = T),
                   sum(polygons61$log_hw_color_hi=="red", na.rm = T))

unflagged_cells <- c(sum(polygons61$metric_color=="white", na.rm = T),
                     sum(polygons61$target_color=="white", na.rm = T),
                     sum(polygons61$count_area_ratio_color=="white", na.rm = T),
                     sum(polygons61$area_color_lo=="white", na.rm = T),
                     sum(polygons61$area_color_hi=="white", na.rm = T),
                     sum(polygons61$log_hw_color_lo=="white", na.rm = T),
                     sum(polygons61$log_hw_color_hi=="white", na.rm = T))

percent_flagged <- round(flagged_cells/dim(polygons61)[1]*100, 2)

poly61 <- data.frame(threshold = c(0.15, 0.15, 0.15, 0.15, 0.85, 0.15, 0.85), flag_value = flag_value, unflagged_cells = unflagged_cells, flagged_cells = flagged_cells, percent_flagged = percent_flagged, row.names = c("Flag score", "Target counts", "Target counts/um area", "Custom area in um low", "Custom area in um high", "Logged aspect ratio low", "Logged aspect ratio high"))

kable(poly61, format = "html", col.names = c("Threshold", "Flag value", "Unflagged cells", "Flagged cells", "% flagged cells"), align = "c", caption = paste0("<center>Flagged cells by different metrics, total cells in fov: ", dim(polygons61)[1], ", total cells in whole sample: ", dim(polygons)[1], ", flagged cells in whole sample: ", sum(spe$flag_score < quantile(spe$flag_score, probs = 0.15, na.rm = T)), "</center>"), padding = 1L)
```
...
Local feature map of flag score, fov 227

```{r, out.width="150%", out.height="150%"}
polygons227 <- filter(polygons, fov==227)

tm_shape(polygons227) + 
  tm_fill(col= "flag_score", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE)
...
```{r, out.width="300%", out.height="900%"}
metric_color <- ifelse(polygons227$flag_score < quantile(spe$flag_score, probs = 0.15), "red", "white")
target_color <- ifelse(polygons227$target_counts < quantile(spe$target_counts, probs = 0.15), "red", "white")
count_area_ratio_color <- ifelse((polygons227$target_counts/polygons227$custom_um_area) < quantile((spe$target_counts/spe$custom_um_area), probs = 0.15), "red", "white")
area_color_lo <- ifelse(polygons227$custom_um_area < quantile(spe$custom_um_area, probs = 0.15), "red", "white")
area_color_hi <- ifelse(polygons227$custom_um_area > quantile(spe$custom_um_area, probs = 0.85), "red", "white")
log_hw_color_lo <- ifelse(polygons227$log2_AspectRatio < quantile(spe$log2_AspectRatio, probs = 0.15), "red", "white")
log_hw_color_hi <- ifelse(polygons227$log2_AspectRatio > quantile(spe$log2_AspectRatio, probs = 0.90), "red", "white")

polygons227 <- cbind(polygons227, metric_color = metric_color, target_color = target_color, count_area_ratio_color = count_area_ratio_color, area_color_lo = area_color_lo, area_color_hi = area_color_hi, log_hw_color_lo = log_hw_color_lo, log_hw_color_hi = log_hw_color_hi)

p1 <- tm_shape(polygons227) + 
  tm_fill(col= "metric_color", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
...
flagged_cells <- c(sum(polygons227$metric_color=="red", na.rm = T),
                   sum(polygons227$target_color=="red", na.rm = T),
                   sum(polygons227$count_area_ratio_color=="red", na.rm = T),
                   sum(polygons227$area_color_lo=="red", na.rm = T),
                   sum(polygons227$area_color_hi=="red", na.rm = T),
                   sum(polygons227$log_hw_color_lo=="red", na.rm = T),
                   sum(polygons227$log_hw_color_hi=="red", na.rm = T))

unflagged_cells <- c(sum(polygons227$metric_color=="white", na.rm = T),
                     sum(polygons227$target_color=="white", na.rm = T),
                     sum(polygons227$count_area_ratio_color=="white", na.rm = T),
                     sum(polygons227$area_color_lo=="white", na.rm = T),
                     sum(polygons227$area_color_hi=="white", na.rm = T),
                     sum(polygons227$log_hw_color_lo=="white", na.rm = T),
                     sum(polygons227$log_hw_color_hi=="white", na.rm = T))

percent_flagged <- round(flagged_cells/dim(polygons227)[1]*100, 2)

poly227 <- data.frame(threshold = c(0.15, 0.15, 0.15, 0.15, 0.85, 0.15, 0.85), flag_value = flag_value, unflagged_cells = unflagged_cells, flagged_cells = flagged_cells, percent_flagged = percent_flagged, row.names = c("Flag score", "Target counts", "Target counts/um area", "Custom area in um low", "Custom area in um high", "Logged aspect ratio low", "Logged aspect ratio high"))

kable(poly227, format = "html", col.names = c("Threshold", "Flag value", "Unflagged cells", "Flagged cells", "% flagged cells"), align = "c", caption = paste0("<center>Flagged cells by different metrics, total cells in fov: ", dim(polygons227)[1], ", total cells in whole sample: ", dim(polygons)[1], ", flagged cells in whole sample: ", sum(spe$flag_score < quantile(spe$flag_score, probs = 0.15, na.rm = T)), "</center>"), padding = 1L)
```
...
Local feature map of flag score, fov 183

```{r, out.width="150%", out.height="150%"}
polygons183 <- filter(polygons, fov==183)

tm_shape(polygons183) + 
  tm_fill(col= "flag_score", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE)
...
```{r, out.width="300%", out.height="900%"}
metric_color <- ifelse(polygons183$flag_score < quantile(spe$flag_score, probs = 0.15), "red", "white")
target_color <- ifelse(polygons183$target_counts < quantile(spe$target_counts, probs = 0.15), "red", "white")
count_area_ratio_color <- ifelse((polygons183$target_counts/polygons183$custom_um_area) < quantile((spe$target_counts/spe$custom_um_area), probs = 0.15), "red", "white")
area_color_lo <- ifelse(polygons183$custom_um_area < quantile(spe$custom_um_area, probs = 0.15), "red", "white")
area_color_hi <- ifelse(polygons183$custom_um_area > quantile(spe$custom_um_area, probs = 0.85), "red", "white")
log_hw_color_lo <- ifelse(polygons183$log2_AspectRatio < quantile(spe$log2_AspectRatio, probs = 0.15), "red", "white")
log_hw_color_hi <- ifelse(polygons183$log2_AspectRatio > quantile(spe$log2_AspectRatio, probs = 0.90), "red", "white")

polygons183 <- cbind(polygons183, metric_color = metric_color, target_color = target_color, count_area_ratio_color = count_area_ratio_color, area_color_lo = area_color_lo, area_color_hi = area_color_hi, log_hw_color_lo = log_hw_color_lo, log_hw_color_hi = log_hw_color_hi)

p1 <- tm_shape(polygons183) + 
  tm_fill(col= "metric_color", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
...
flagged_cells <- c(sum(polygons183$metric_color=="red", na.rm = T),
                   sum(polygons183$target_color=="red", na.rm = T),
                   sum(polygons183$count_area_ratio_color=="red", na.rm = T),
                   sum(polygons183$area_color_lo=="red", na.rm = T),
                   sum(polygons183$area_color_hi=="red", na.rm = T),
                   sum(polygons183$log_hw_color_lo=="red", na.rm = T),
                   sum(polygons183$log_hw_color_hi=="red", na.rm = T))

unflagged_cells <- c(sum(polygons183$metric_color=="white", na.rm = T),
                     sum(polygons183$target_color=="white", na.rm = T),
                     sum(polygons183$count_area_ratio_color=="white", na.rm = T),
                     sum(polygons183$area_color_lo=="white", na.rm = T),
                     sum(polygons183$area_color_hi=="white", na.rm = T),
                     sum(polygons183$log_hw_color_lo=="white", na.rm = T),
                     sum(polygons183$log_hw_color_hi=="white", na.rm = T))

percent_flagged <- round(flagged_cells/dim(polygons183)[1]*100, 2)

poly183 <- data.frame(threshold = c(0.15, 0.15, 0.15, 0.15, 0.85, 0.15, 0.85), flag_value = flag_value, unflagged_cells = unflagged_cells, flagged_cells = flagged_cells, percent_flagged = percent_flagged, row.names = c("Flag score", "Target counts", "Target counts/um area", "Custom area in um low", "Custom area in um high", "Logged aspect ratio low", "Logged aspect ratio high"))


kable(poly183, format = "html", col.names = c("Threshold", "Flag value", "Unflagged cells", "Flagged cells", "% flagged cells"), align = "c", caption = paste0("<center>Flagged cells by different metrics, total cells in fov: ", dim(polygons183)[1], ", total cells in whole sample: ", dim(polygons)[1], ", flagged cells in whole sample: ", sum(spe$flag_score < quantile(spe$flag_score, probs = 0.15, na.rm = T)), "</center>"), padding = 1L)
```
...
Local feature map of flag score, fov 123

```{r, out.width="150%", out.height="150%"}
polygons123 <- filter(polygons, fov==123)

tm_shape(polygons123) + 
  tm_fill(col= "flag_score", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE)
...
```{r, out.width="300%", out.height="900%"}
metric_color <- ifelse(polygons123$flag_score < quantile(spe$flag_score, probs = 0.15), "red", "white")
target_color <- ifelse(polygons123$target_counts < quantile(spe$target_counts, probs = 0.15), "red", "white")
count_area_ratio_color <- ifelse((polygons123$target_counts/polygons123$custom_um_area) < quantile((spe$target_counts/spe$custom_um_area), probs = 0.15), "red", "white")
area_color_lo <- ifelse(polygons123$custom_um_area < quantile(spe$custom_um_area, probs = 0.15), "red", "white")
area_color_hi <- ifelse(polygons123$custom_um_area > quantile(spe$custom_um_area, probs = 0.85), "red", "white")
log_hw_color_lo <- ifelse(polygons123$log2_AspectRatio < quantile(spe$log2_AspectRatio, probs = 0.15), "red", "white")
log_hw_color_hi <- ifelse(polygons123$log2_AspectRatio > quantile(spe$log2_AspectRatio, probs = 0.90), "red", "white")

polygons123 <- cbind(polygons123, metric_color = metric_color, target_color = target_color, count_area_ratio_color = count_area_ratio_color, area_color_lo = area_color_lo, area_color_hi = area_color_hi, log_hw_color_lo = log_hw_color_lo, log_hw_color_hi = log_hw_color_hi)

p1 <- tm_shape(polygons123) + 
  tm_fill(col= "metric_color", alpha = 0.5) +
  tm_borders(lwd = 0.1, col = "grey50") +
  tm_layout(legend.outside = TRUE,
            main.title.position = c("center", "top"),
...
flagged_cells <- c(sum(polygons123$metric_color=="red", na.rm = T),
                   sum(polygons123$target_color=="red", na.rm = T),
                   sum(polygons123$count_area_ratio_color=="red", na.rm = T),
                   sum(polygons123$area_color_lo=="red", na.rm = T),
                   sum(polygons123$area_color_hi=="red", na.rm = T),
                   sum(polygons123$log_hw_color_lo=="red", na.rm = T),
                   sum(polygons123$log_hw_color_hi=="red", na.rm = T))

unflagged_cells <- c(sum(polygons123$metric_color=="white", na.rm = T),
                     sum(polygons123$target_color=="white", na.rm = T),
                     sum(polygons123$count_area_ratio_color=="white", na.rm = T),
                     sum(polygons123$area_color_lo=="white", na.rm = T),
                     sum(polygons123$area_color_hi=="white", na.rm = T),
                     sum(polygons123$log_hw_color_lo=="white", na.rm = T),
                     sum(polygons123$log_hw_color_hi=="white", na.rm = T))

percent_flagged <- round(flagged_cells/dim(polygons123)[1]*100, 2)

poly123 <- data.frame(threshold = c(0.15, 0.15, 0.15, 0.15, 0.85, 0.15, 0.85), flag_value = flag_value, unflagged_cells = unflagged_cells, flagged_cells = flagged_cells, percent_flagged = percent_flagged, row.names = c("Flag score", "Target counts", "Target counts/um area", "Custom area in um low", "Custom area in um high", "Logged aspect ratio low", "Logged aspect ratio high"))


kable(poly123, format = "html", col.names = c("Threshold", "Flag value", "Unflagged cells", "Flagged cells", "% flagged cells"), align = "c", caption = paste0("<center>Flagged cells by different metrics, total cells in fov: ", dim(polygons123)[1], ", total cells in whole sample: ", dim(polygons)[1], ", flagged cells in whole sample: ", sum(spe$flag_score < quantile(spe$flag_score, probs = 0.15, na.rm = T)), "</center>"), padding = 1L)
```
