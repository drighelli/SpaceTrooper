---
title: "06_04_cosmx_dbkero_BC_custom_mc_plots"
...
```{r}
spe <- readCosmxSPE(dirname = dirname, fov_dims=c(xdim=4225, ydim=4225))
...
for(k in 1:length(terra_list)){        
  ...
  spatVec_dt$x_global_px <- spatVec_dt$x_local_px + metadata(spe)$fov_positions$x_global_px[k]
  spatVec_dt$y_global_px <- spatVec_dt$y_local_px +metadata(spe)$fov_positions$y_global_px[k]
  # add a column with the fov and transform into a data.table
  spatVec_dt <- data.frame(fov =metadata(spe)$fov_positions[k,"fov"], as.data.frame(spatVec_dt))
  spat_obj <- rbind(spat_obj, spatVec_dt)
}

spat_obj$fov_id <- as.factor(paste(spat_obj$fov, spat_obj$cell_id, sep = "_"))

tictoc::tic()
polygons_whole_custom <- sfheaders::sf_polygon(obj = spat_obj, x = "x_global_px", y = "y_global_px", polygon_id = "fov_id", keep = TRUE) #1.899 sec elapsed
tictoc::toc()
...
```{r}
polygons <- sfheaders::sf_polygon(obj = spat_obj, x = "x_local_px", y = "y_local_px", polygon_id = "fov_id", keep = TRUE) #1.899 sec elapsed
...
out_boundaries <- c(0, 0.1, 
               round(area_fence[1,1], 2), 
               round(area_fence[2,1], 2),
               round(dapi_fence[1,1], 2), 
               round(dapi_fence[2,1], 2))

flagged_cells <- c(sum(spe$flagged_0total == TRUE, na.rm = T),
                   sum(spe$outlier_ctrl_tot == TRUE, na.rm = T),
                   sum(spe$um_area < round(area_fence[1,1], 2), na.rm = T), 
                   sum(spe$um_area > round(area_fence[2,1], 2), na.rm = T), 
                   sum(spe$Mean.DAPI < round(dapi_fence[1,1], 2),na.rm = T),
                   sum(spe$Mean.DAPI > round(dapi_fence[2,1], 2),na.rm = T))

unflagged_cells <- c(sum(spe$flagged_0total == FALSE, na.rm = T),
                     sum(spe$outlier_ctrl_tot == FALSE, na.rm = T),
                     sum(spe$umarea_outlier == FALSE,na.rm = T),
                     " ",
                     sum(spe$dapi_outlier == FALSE, na.rm = T),
                     " ")

union_flagged <- length(unique(c(spe[,spe$flagged_0total == TRUE]$fov_id,
                   spe[,spe$outlier_ctrl_tot == TRUE]$fov_id,
                   spe[,spe$umarea_outlier == TRUE]$fov_id,
                   spe[,spe$dapi_outlier == TRUE]$fov_id)))

percent_flagged <- round(flagged_cells/dim(spe)[2]*100, 2)

stat_df <- data.frame(out_boundaries = out_boundaries, unflagged_cells = unflagged_cells, flagged_cells = flagged_cells, percent_flagged = percent_flagged, row.names = c("Total counts", "Control/total counts ratio", "Cell area in um < lower thr.", "Cell area in um > higher thr.", "Mean DAPI signal < lower thr.", "Mean DAPI signal > higher thr."))
