...
for (var in 1:length(variables)){
  ...
  p <- ggplot(data = data.frame(colData(spe))) +
    geom_histogram(aes(x = .data[[n]]), fill = "#69b3a2") +
    ggtitle(paste0(n))
  print(p)
}  
```
...
# TOTAL COUNT >= 20th PERCENTILE FOR 1000-PLEX, >= 5th PERCENTILE FOR 100-PLEX
...
for (var in 1:length(heatmap_variables)){
  ...
  geom_text(data = fov_positions, aes(x = fov_positions$x_global_px+fov_size/2,
    ...
    ggtitle(paste0(heat_var, " by_fov"))
  print(p)
}  
```
